...
function filterBrands() {
  ...
  if (!prov) {
    ...
    cards.forEach(function(card) {
      ...
      if (card.dataset.def === 'hid') {
        card.classList.add('oculto');   // vuelve a ocultarse
      } else {
        card.classList.remove('oculto'); // los 5 destacados quedan visibles
      }
    });
    if (moreBtn) moreBtn.style.display = '';
  } else {
    // Con provincia: mostrar TODOS los comercios de esa provincia
    cards.forEach(function(card) {
      card.classList.remove('oculto');
      if ((card.dataset.prov || '') === prov) {
        card.style.display = '';
        count++;
      } else {
  ...
  if (countEl) {
    countEl.textContent = !prov ? ''
      : (count === 0 ? 'Sin resultados para esta provincia'
                     : 'Mostrando ' + count + ' comercio' + (count === 1 ? '' : 's'));
  }

  // Estado "sin resultados"
  var noResults = document.getElementById('brands-no-results');
  if (noResults) noResults.style.display = (prov && count === 0) ? '' : 'none';
}
